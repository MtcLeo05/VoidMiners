...
public class SolarControllerRenderer implements BlockEntityRenderer<SolarControllerBE> {

    public SolarControllerRenderer(BlockEntityRendererProvider.Context context) {
    }
    ...
    public void render(SolarControllerBE pBlockEntity, float pPartialTick, PoseStack pose, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        if (pBlockEntity.active) {
            ...
            // Scale beam length by current solar multiplier (day/night + weather)
            float mult = pBlockEntity.getSolarMultiplier();
            if (mult > 0.001f) {
                float length = 64f + 256f * mult; // shorter at dawn/dusk, tallest at noon

                pose.pushPose();
                pose.translate(0.5f, 0, 0.5f);
                pose.mulPose(Axis.YP.rotationDegrees(f * 2.25f - 45f));

                // Render the beam upward to suggest energy generation
                renderBeam(pBuffer.getBuffer(RenderType.gui()), pose, new Vector3f(0f, 0f, 0f), pBlockEntity.getBeamColor(), length, 0.3f);
        ...
        for (int x = 0; x < blocks.size(); x++) {
            ...
            for (int y = 0; y < b2.size(); y++) {
                ...
                for (int z = 0; z < b3.size(); z++) {
                    ...
                    renderBlock(block, pose, pBuffer);
                    pose.popPose();
                }
            }
        }
